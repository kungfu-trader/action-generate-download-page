...
const generateHTML = async (argv) => {
  ...
  for (const artifactName of artifacts.length > 0 ? artifacts : pkgInfo) {
    ...
    const { timestamp } = await createPage(options, downloadBaseUrl);
    await createMetadata(options, version, timestamp);
...
const createPage = async (argv, downloadBaseUrl) => {
  ...
  const output = mustache.render(template, {
    ...
    title: `下载${argv.artifactName} ${argv.version} - 功夫量化`,
    menu: getMenu(argv.artifactName),
  });
  const fileName = path.join(process.cwd(), `${htmlDir}/index.html`);
  writeFile(fileName, output, htmlDir);
  return {
    timestamp: Date.parse(
      tableItem[0].date ? new Date(tableItem[0].date) : new Date()
    ),
  };
};
...
const createMetadata = async (argv, version, timestamp) => {
  if (!argv.apiKey) {
    return;
  }
